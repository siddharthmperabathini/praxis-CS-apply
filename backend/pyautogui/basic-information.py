...
try:
    ...
    # Process each field
    num_fields = 9  # number of fields to process
    pyautogui.scroll(-4)
    ...
    for i in range(num_fields):
        ...
        if detected_box is None:
            ...
            # Look at a small region inside the box to see if there's visible content
            inner_box = image_bgr[y + 5 : y + h_box - 5, x + 5 : x + w_box - 5]
            inner_gray = cv2.cvtColor(inner_box, cv2.COLOR_BGR2GRAY)

            # Calculate pixel intensity statistics
            mean_val = np.mean(inner_gray)
            std_val = np.std(inner_gray)

            # If there's content, the box likely has some pixel variation
            if (
                std_val > 10 and mean_val < 240
            ):  # Adjust these thresholds based on testing
                print(
                    f"[INFO] Field #{i}: Field appears to be filled already, skipping"
                )
        ...
        # time.sleep(2)  # Wait before scrolling
        pyautogui.scroll(-3)
...
# add a feature to do next button to transition to next page
